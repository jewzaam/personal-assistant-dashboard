...
- **`tasks_md.py`** — parser and write-back for `<pa_workspace>/actions.md`, the PA's markdown task list. Headings are sections, bullets are tasks, deeper-indented bullets are detail lines on the task above. A bullet with no checkbox at all is an open task, so the PA needs no format change. `Task.key` is the bullet text **only** — the PA reshuffles actions.md by due date, and a task moving from a date section to `(TODAY)` to `Overdue` must not read as new. `set_done()` and `move_task()` re-read the file and refuse the write when the target line no longer matches the task, so a concurrent PA edit is never clobbered. actions.md goes stale whenever it outlives the day the PA wrote it, so the `(TODAY)` marker in the file is never trusted: `section_label()` recomputes the heading's marker from the real date for display — a stale `(TODAY)` renders `(OVERDUE)`, and today's section gets marked even when the file did not — and `normalize_today_markers()` moves the marker onto the real today's heading on any write, which is what stops a bump-to-today beside a stale marker leaving **two** `(TODAY)` headings. It never creates a section for today: an empty one is clutter, and `move_task()` already writes it when a task lands there. Nothing is rewritten on load — the PA owns the file's organization, the dashboard only corrects markers on writes the user triggered. `bump_target()` is the arrow rule: today is the floor in both directions, so a named bucket ("Overdue", "Demos to watch") only moves down and lands on today, and steps skip weekends. `move_task()` carries the task's detail lines with it, creates a missing date section in date order above the named buckets, and drops a dated section it empties — named buckets stay, they are structure the user maintains
...
- **`update_idletasks()` after `delete("all")` flickers:** Tk schedules a canvas repaint as an idle handler, so `update_idletasks()` on an emptied canvas paints a blank frame before the redraw fills it. Call it *before* the delete — `_render_current_day()` needs it only to get an accurate `winfo_width()`, which the earlier call gives just as well.
