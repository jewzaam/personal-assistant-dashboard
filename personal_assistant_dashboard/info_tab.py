...
"""INFO tab — displays per-process data from AgentPulse.

Subscribes to an AgentPulseClient for real-time session updates.
Claude Code reports cost/tokens cumulatively per OS process, so
``/clear`` rotations inside a single CLI process produce multiple
session_ids sharing the same counters. Sessions are collapsed by
``(source_system, pid)`` — one row per live process plus an
aggregate summary.
"""
...
class InfoTab:
    ...
    @staticmethod
    def _collapse_by_process(
        sessions: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Aggregate sessions by ``(source_system, pid)``.

        Claude Code tracks cost/tokens cumulatively per process, so every
        ``/clear`` issues a new ``session_id`` while the counters carry
        over. Within each group, keep the session with the highest
        ``last_event_at`` (fallback ``started_at``) as the process
        snapshot and adopt the earliest ``started_at`` in the group so
        the row represents total process uptime.
        """
        groups: dict[Any, list[dict[str, Any]]] = {}
        for s in sessions:
            pid = s.get("pid") or 0
            if not pid:
                # Can't safely aggregate without a pid — leave standalone
                groups[("__unique__", id(s))] = [s]
                continue
            source = s.get("source_system") or ""
            groups.setdefault((source, pid), []).append(s)
        ...
        for group in groups.values():
            newest = max(
                group,
                key=lambda s: (
                    s.get("last_event_at") or 0.0,
                    s.get("started_at") or 0,
                ),
            )
            collapsed = dict(newest)
            starts = [s.get("started_at") or 0 for s in group if s.get("started_at")]
            if starts:
                collapsed["started_at"] = min(starts)
...
def _process_key(session: dict[str, Any]) -> str:
    """Return a display key for a process row: ``<pid>+<host>``.

    Shows ``\u2014`` when the pid is unknown so the column stays aligned.
    """
    pid = session.get("pid") or 0
    host = session.get("source_system") or ""
    if not pid:
        return "\u2014"
    return f"{pid}+{host}" if host else str(pid)
